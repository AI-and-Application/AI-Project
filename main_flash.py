...
class MyLitModel(pl.LightningModule):
    def __init__(self, audio_model_name, num_label2s):
        ...
        # Flash Attention을 사용하도록 attention 계층 수정
        for layer in self.audio_model.hubert.encoder.layers:
            ...
            def forward_with_flash_attention(hidden_states, attention_mask=None, output_attentions=False, **kwargs):
                ...
                # QKV 계산
                query = original_attention.q_proj(hidden_states)
                key = original_attention.k_proj(hidden_states)
                value = original_attention.v_proj(hidden_states)
                
                # 텐서 재구성
                hidden_size = query.size(-1)
                num_heads = original_attention.num_heads  # num_attention_heads를 num_heads로 수정
                ...
                value = value.view(batch_size, seq_length, num_heads, head_dim).transpose(1, 2)
                
                # Flash Attention 적용
                attn_output = flash_attn_func(
                    query, key, value,
                    dropout_p=0.0 if not self.training else original_attention.dropout,
                    softmax_scale=None,
                    causal=False
                )
                ...
                # 출력 프로젝션
                attn_output = original_attention.out_proj(attn_output)
...
def analyze_emotion(audio_path):
    ...
    # 입력을 GPU로 이동하고 float16으로 변환
    audio_values = inputs["input_values"].to(hubert_model.device).half()
...
classify_prompt = ChatPromptTemplate.from_messages(
    [
        SystemMessagePromptTemplate.from_template(
            ...
            - Respond with a comma-separated list of up to 2 relevant location-related keywords in English
            - This includes:
                * Direct navigation requests (e.g., "How do I get to...")
                * Location finding requests (e.g., "Where is the nearest...")
                * Queries about finding specific places (e.g., "Is there a... nearby?")
                * Indirect navigation needs (e.g., "I need to find...", "Looking for...")
                * Questions about locations or facilities in an area
            ...
            Output: Gangnam Station

            3. Input: Why is that car interrupting all of a sudden? You didn't turn on the blinkers!
            Output: False
            ...
            6. Input: Where can I find a park near Seoul?
            Output: park, Seoul
...
async def analyze_request(request: Request):
    ...
    if classify_result.lower() == "false":
        ...
        if len(search_keywords) <= 2:
            # This is a location/navigation request
            locations = ",".join(search_keywords)
            additional_info = locations
            print("navigation")
            #여기서 그냥 나가면 될듯,,? 여기로 들어오면 그냥 내비 api 다시 불러와서 경로 다시 검색하는걸루... 

        else:
            # This is a general search request
            search_query = " ".join(search_keywords)
            additional_info = get_google_results(search_query)
